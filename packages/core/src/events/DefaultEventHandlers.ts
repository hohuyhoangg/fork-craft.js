import { CraftDOMEvent, isChromium, isLinux } from '@craftjs/utils';
      ...
      drag: (el: HTMLElement, id: NodeId) => {
        ...
        const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
          ...
          this.dropElement((dragTarget, indicator) => {
            ...
            store.actions.setProp(dragTarget.nodes[0], (props) => {
              props['top'] = Math.floor(indicator.placement.ry / 25) * 25;
              props['left'] = Math.floor(indicator.placement.rx / 25) * 25;
              // props['top'] = indicator.placement.ry;
              // props['left'] = indicator.placement.rx;
              // props['width'] = indicator.placement.w;
              // props['height'] = indicator.placement.h;
      ...
      create: (
        ...
        const unbindDragStart = this.addCraftEventListener(
          ...
          (e) => {
            ...
            console.log('🚀 ~ handlers ~ tree:', tree);
        ...
        const unbindDragEnd = this.addCraftEventListener(el, 'dragend', (e) => {
          ...
          this.dropElement((dragTarget, indicator) => {
            ...
            const index = indicator.placement.index + 1;

            console.log(
              '🚀 ~ this.dropElement ~ indicator.placement:',
              indicator.placement
            );
            ...
            if (options && isFunction(options.onCreate)) {
              options.onCreate(
                dragTarget.tree,
                indicator.placement.rx,
                indicator.placement.ry,
                indicator.placement.w,
                indicator.placement.h
              );
            }
          }, e);
        });
